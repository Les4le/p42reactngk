...
export default function Cart() {
  ...
  const saving = fullPrice - cart.price;
      ...
      <div className="col col-8">
        ...
        {cart.cartItems.length == 0 ? (
          ...
          cart.cartItems.map((ci) => (
            <CartItemView key={ci.product.id} ci={ci} />
          ))
        )}
      </div>
      <div className="col col-4">
        <div className="mt-2 bg-light border p-0">
          <h3 className="bg-body-tertiary border-bottom py-2 text-center">
            Order summary
          </h3>
          <div className="d-flex justify-content-between mx-2 mb-2">
            <span>Subtotal</span>
            <b>
              {cart.cartItems.reduce((acc, x) => acc + x.price, 0.0).pad2()}
            </b>
          </div>
          ...
          <>
            {saving > 0 && (
              <div className="d-flex justify-content-between px-2 text-success fw-bold">
                <span>Вигода</span>
                <span>-₴{saving.pad2()}</span>
              </div>
            )}
...
function CartItemView({ ci }: { ci: ICartItem }) {
  ...
  const onQuantityChange = (quantity: number) => {
    if (quantity > 0) {
      ...
      if (window.confirm("Видалити товар із кошика?")) {
        cart.cartItems = cart.cartItems.filter(
          (x) => x.product.id !== ci.product.id,
        );
    ...
    <div className="row mb-3">
      ...
      <div className="col col-1 col-sm-2 text-center">
        <span className="position-relative">
          ...
          {ci.price < fullPrice && (
            <div className="strike-ci-price">{fullPrice.pad2()}</div>
          )}
        </span>
      </div>
    </div>
